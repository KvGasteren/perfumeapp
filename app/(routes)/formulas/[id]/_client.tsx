...
export function FormulaEditorClient({
  ...
  initialRows,
  ingredientOptions,
  initialAllergenCache,
}: {
  formula: Formula
  initialRows: FormulaIngredientLink[]
  ingredientOptions: Ingredient[]
  initialAllergenCache: Record<
    number,
  ...
  function removeRow(idx: number) {
    ...
    setRows((prev) => {
      ...
      return prev.map((r, i) => (i === idx ? { ...r, _status: 'deleted' } : r))
    })
  }

  /**
   * NEW: only fetch allergens for rows that...
   * - are active
   * - have a numeric ingredientId
   * - are NOT yet in allergenCache
   *
   * This means: no burst of calls on initial load, because initial cache
   * already contains all original formula ingredients.
   */
  useEffect(() => {
    const missingIds = rows
      .filter(
        (r) =>
          ...
          !allergenCache[r.ingredientId]
      )
      .map((r) => r.ingredientId as number)

    if (missingIds.length === 0) return

    Promise.all(missingIds.map((id) => fetchIngredientAllergens(id)))
      .then((results) => {
  ...
  const allergenSummary = useMemo(() => {
    ...
    const totalParts = activeRows.reduce((sum, r) => sum + Number(r.parts), 0)
  ...
  function handleCancel() {
    ...
    setEditMode(false)
    // also reset allergen cache to what we got from the server
    setAllergenCache(initialAllergenCache)
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={editMode ? `Edit ${formula.name}` : `${formula.name}`}
        actions={
